# Adapted from ARC (https://github.com/msesia/arc), MIT License.
# See third_party/arc/LICENSE.
# Changes by <Yin Xun Kho>, 2025: <Modified the method to initialize and force the class label universe to np.arange(1000) (instead of inferring from data) so prediction sets are computed over a fixed 1000-class space for Imagenet.>.
...
class JackknifePlus:
    def __init__(self, X, Y, black_box, alpha, random_state=2020, allow_empty=True, verbose=False):
        ...
        self.n = X.shape[0]
        self.classes = np.unique(Y) 
        #self.n_classes = len(self.classes) #delete
        self.alpha = alpha
        self.allow_empty = allow_empty
        self.verbose = verbose
...
class SplitConformal:
    def __init__(self, X, Y, black_box, alpha, random_state=2020, allow_empty=True, verbose=False):
        ...
        self.black_box = black_box.fit(X_train, Y_train)
...
class SplitConformalTwo:
    def __init__(self, X=None, Y=None, black_box=None, alpha=0.1,
                 *,                # force keyword-only extras
                 X_train=None, Y_train=None, X_calib=None, Y_calib=None,
                 test_size=0.5, random_state=2020, allow_empty=True, verbose=False):
        self.allow_empty = allow_empty
        self.black_box = black_box
        rng = np.random.default_rng(random_state)

        # --- choose split mode ---
        if X_train is None or Y_train is None or X_calib is None or Y_calib is None:
            # internal split (backwards compatible)
            X_train, X_calib, Y_train, Y_calib = train_test_split(
                X, Y, test_size=test_size, random_state=random_state, stratify=Y
            )

        # --- fit on TRAIN ---
        self.black_box = self.black_box.fit(X_train, Y_train)
    ...
    def predict(self, X, random_state=2020):
        ...
        p_hat = self.black_box.predict_proba(X)
        grey_box = ProbAccum(p_hat)
        return grey_box.predict_sets(self.alpha_calibrated, epsilon=epsilon, allow_empty=self.allow_empty)
