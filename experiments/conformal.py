# Adapted from RAPS (MIT License), copyright (c) 2020 Anastasios Angelopoulos.
...
class ConformalModel(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        ...
        self.model = model 
        self.alpha = alpha
        self.T = torch.Tensor([1.3]).to(device) #initialize (1.3 is usually a good value)
        self.T, calib_logits = platt(self, calib_loader)
        self.randomized=randomized
        self.allow_zero_sets=allow_zero_sets
        #self.num_classes = calib_loader.dataset.tensors[1].max().item() + 1
        #this is for tabular data
        #self.num_classes = len(calib_loader.dataset.dataset.classes)
        #self.num_classes = len(calib_loader.dataset.classes) if hasattr(calib_loader.dataset, 'classes') else len(calib_loader.dataset.dataset.classes)
        #this is for imagenet with RAPS
        if hasattr(calib_loader.dataset, 'classes'):
            self.num_classes = len(calib_loader.dataset.classes)
        elif hasattr(calib_loader.dataset, 'tensors'):
            self.num_classes = int(calib_loader.dataset.tensors[1].max().item()) + 1
        elif hasattr(calib_loader.dataset, 'dataset') and hasattr(calib_loader.dataset.dataset, 'classes'):
            self.num_classes = len(calib_loader.dataset.dataset.classes)
        else:
            raise ValueError("Could not determine number of classes from calib_loader.dataset")
...
def conformal_calibration(cmodel, calib_loader):
    ...
    with torch.no_grad():
        ...
        for x, targets in tqdm(calib_loader):
            logits = cmodel.model(x.to(device)).detach().cpu().numpy() #x has cuda
...
class ConformalModelLogits(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, naive=False, LAC=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        ...
        if not naive and not LAC:
            ...
            gt_locs_cal = np.array([np.where(np.argsort(x[0])[::-1] == x[1])[0][0] for x in calib_loader.dataset]) 
...
def conformal_calibration_logits(cmodel, calib_loader):
    with torch.no_grad():
        ...
        for logits, targets in calib_loader:
            ...
            E = np.concatenate((E,giq(scores,targets,I=I,ordered=ordered,cumsum=cumsum,penalties=cmodel.penalties,randomized=cmodel.randomized,allow_zero_sets=cmodel.allow_zero_sets))) #it was true true for zero sets and randomized
...
def platt_logits(cmodel, calib_loader, max_iters=10, lr=0.01, epsilon=0.01):
    ...
    T = nn.Parameter(torch.Tensor([1.3]).to(device)) #.cuda() after the second last bracket
    ...
    for iter in range(max_iters):
        ...
        for x, targets in calib_loader:
            ...
            out = x/T
            loss = nll_criterion(out, targets.long().to(device)) #.cuda() after long()
            loss.backward()
            optimizer.step()
...
def gcq(scores, tau, I, ordered, cumsum, penalties, randomized, allow_zero_sets):
    ...
    #penalties_cumsum = cumsum + penalties
...
def pick_kreg(paramtune_logits, alpha):
    ...
    gt_locs_kstar = np.array([np.where(np.argsort(x[0])[::-1] == x[1])[0][0] for x in paramtune_logits])
...
def pick_lamda_adaptiveness(model, paramtune_loader, alpha, kreg, randomized, allow_zero_sets, strata = [[0,1], [2,3], [4,6], [7,10]]):
    # Calculate lamda_star
    lamda_star = 0
    best_violation = 1
    # Use the paramtune data to pick lamda.  Does not violate exchangeability.
