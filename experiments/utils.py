...
def validate(val_loader, model, print_bool):
    with torch.no_grad():
        ...
        for i, (x, target) in enumerate(val_loader):
            ...
            # compute output
            output, S = model(x.to(device)) #x.cuda()
...
def data2tensor(data):
    imgs = torch.cat([x[0].unsqueeze(0) for x in data], dim=0).to(device) #.cuda()
    targets = torch.cat([torch.Tensor([int(x[1])]) for x in data], dim=0).long().to(device)
    return imgs, targets
...
def get_logits_targets(model, loader):
    # Get num_classes from the model's output or the data
    with torch.no_grad():
        for x, _  in loader:
            #n_classes = model(x.to(x.device if hasattr(x, "device") else "cpu")).shape[1] THIS IS FOR TABULAR
            #HERE WAS CHANGE FOR THE IMAGE ONE SO PLEASE TAKE NOTEEEE
            device = next(model.parameters()).device
            n_classes = model(x.to(device)).shape[1]
            break
    ...
    with torch.no_grad():
        for x, targets in loader:
            #batch_logits = model(x.to(x.device if hasattr(x, "device") else "cpu")).detach().cpu()
            device = next(model.parameters()).device
